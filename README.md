...
- Browser-control features on Windows require the Chrome DevTools MCP server in your Claude Code user config:
...
| `VITE_KOKORO_VOICE` | Voice id for Kokoro-based frontend TTS |
| `VITE_USE_ELEVENLABS` | Force ElevenLabs TTS on |
| `VITE_PICOVOICE_ACCESS_KEY` | Enables Porcupine wake-word mode |
| `VITE_ANTHROPIC_API_KEY` | Advanced frontend API key setting outside the bridge-first flow |
